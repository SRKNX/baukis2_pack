...
describe "職員による自分のアカウント編集","ログイン前" do
  include_examples "a protected singular staff controller", "staff/accounts"
  # spec/support下ファイルのinclude_examples "内容",検証したい
end


describe "職員による自分のアカウント編集" do

  # accounts_controllerでいうところのcurrent_staff_memberにあたるデータに値がないため、
  # テストを作動させる前にここで手動で作る必要がある。
  # 下の処理は、架空の職員がメールアドレスとパスワードを入れてログインする動作を再現している。
