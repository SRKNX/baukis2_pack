...
describe Staff::Authenticator do
  describe "#authenticate" do
    example "正しいパスワードならtrueを返す" do
      ...
      expect(Staff::Authenticator.new(m).authenticate("foobar")).to be_truthy
    end

    example "誤ったパスワードならfalseを返す" do
      m = build(:staff_member)
      expect(Staff::Authenticator.new(m).authenticate("baseball")).to be_falsey
    end
    ...
    example "停止フラグが立っていればfalseを返す" do
      ...
      expect(Staff::Authenticator.new(m).authenticate("foobar")).to be_falsey
    end
